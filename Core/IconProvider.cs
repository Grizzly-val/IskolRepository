...
public sealed class IconProvider
{
        ...
        // Warning icon keys (generated by appending _warning to base keys)
        public const string FolderWarningIconKey = "folder_warning";
        public const string FileWarningIconKey = "file_warning";
        public const string ImageWarningIconKey = "image_warning";
        public const string VideoWarningIconKey = "video_warning";
        ...
        public ImageList CreateImageList()
        {
            ...
            imageList.Images.Add(FolderWarningIconKey, AddWarningIndicator(CreateFolderIcon()));
            imageList.Images.Add(FileIconKey, CreateFileIcon());
            imageList.Images.Add(FileWarningIconKey, AddWarningIndicator(CreateFileIcon()));
            imageList.Images.Add(ImageIconKey, CreateImageIcon());
            imageList.Images.Add(ImageWarningIconKey, AddWarningIndicator(CreateImageIcon()));
            imageList.Images.Add(VideoIconKey, CreateVideoIcon());
            imageList.Images.Add(VideoWarningIconKey, AddWarningIndicator(CreateVideoIcon()));
    ...
    /// <summary>
    /// Gets the appropriate icon key for any item (file or folder) with optional validation warning.
    /// This is the primary method for determining node icons in tree views.
    /// </summary>
    /// <param name="itemPath">Path to the item (file or folder)</param>
    /// <param name="hasValidationIssue">Whether the item has a validation warning</param>
    /// <returns>The appropriate icon key, with warning applied if needed</returns>
    public string GetIconKeyWithValidation(string itemPath, bool hasValidationIssue = false)
    {
        var baseIconKey = GetIconKey(itemPath);
        return ApplyWarningToIconKey(baseIconKey, hasValidationIssue);
    }
    ...
    {
        return ApplyWarningToIconKey(FolderIconKey, hasWarning);
    }

    /// <summary>
    /// Gets the appropriate icon key for any icon type, optionally with a warning indicator.
    /// This method centralizes the logic for applying warning indicators to any icon type.
    /// </summary>
    /// <param name="baseIconKey">The base icon key (e.g., FolderIconKey, FileIconKey, ImageIconKey, VideoIconKey)</param>
    /// <param name="hasWarning">Whether to apply a warning indicator to the icon</param>
    /// <returns>The appropriate icon key, with warning suffix if hasWarning is true</returns>
    public string ApplyWarningToIconKey(string baseIconKey, bool hasWarning = false)
    {
        if (!hasWarning)
            return baseIconKey;

        return baseIconKey switch
        {
            FolderIconKey => FolderWarningIconKey,
            FileIconKey => FileWarningIconKey,
            ImageIconKey => ImageWarningIconKey,
            VideoIconKey => VideoWarningIconKey,
            _ => baseIconKey  // Return base key if warning variant not available
        };
    }
}
